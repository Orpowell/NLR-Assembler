...
def filter_by_strand(strand_info, annotation_info, contig_groups):
    """
    ...
    strand_sorted = seperated_stranded_groups + singletons + same_stranded_groups

    singles = [val for val in strand_sorted if len(val) == 1]
    groups = [val for val in strand_sorted if len(val) > 1]
...
def annotate_grouped_contigs(sorted_contigs, annotation_dictionary, error_mapping):
    ...
    if error_mapping is not None:
        ...
        logging.info(
            f"Mismatched contigs: {100 * mismatch_count / total_count:.3}% ({mismatch_count} of {total_count})")
        logging.info(
            f"Total contig errors: {100 * (unknown_count + mismatch_count) / total_count:.3}% ({(unknown_count + mismatch_count)} of {total_count})")
...
def annotate(input_data, nlr, assembly_contigs, error_mapping_data):
    ...
    strand_filtered_contigs = filter_by_strand(contig_strands, contig_annotations, contigs)
